...
using WebApiFreightFinder.Models;
using WebApiFreightFinder.Providers;
using WebApiFreightFinder.Results;
...
namespace WebApiFreightFinder.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Register(RegisterUserDto registerUserDto)
        {
            ...
            var user = new ApplicationUser() { UserName = registerUserDto.RegisterModel.UserName, Email = registerUserDto.RegisterModel.Email };

            IdentityResult result = await UserManager.CreateAsync(user, registerUserDto.RegisterModel.Password);
           ...
           if(result.Succeeded) 
            {
                ...
                try
                {
                    ...
                    registerUserDto.User.Id = user.Id;
                    _userServices.Add(registerUserDto.User);
        ...
        public async Task<IHttpActionResult> ForgotPassword(ChangePasswordModel model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    var callbackUrl = new Uri(Url.Link("ForgotPasswordConfirmation", new { userId = user.Id, code = code }));                
         ...
         [System.Web.Http.AllowAnonymous]
         [System.Web.Http.Route("ForgotPasswordConfirmation", Name = "ForgotPasswordConfirmation")]
        public async Task<IHttpActionResult> ForgotPasswordConfirmation(string userId, string code)
       {
            return Ok();
        }
        ...
        public async Task<IHttpActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            if (result.Succeeded)
            {
                _userServices.ConfirmUser(userId);
                return Ok();
            }
            else
            {
